...
7. OUTPUT FORMATTING:
   ...
   - Display user information after succeGssful operations
...
user_db =[('Name',name), ('password',password),('Balance',balance),('is_verified',is_verified)]
if command == "login":
   if user_name == name:
      if passw == password:
         print("login successful your details are",user_db)
         quit()
      else:
         if user_name and passw != name and password:
            print("invalid login details")
   else:
       print("invalid login details kindly Register")
else:
  if command == "register" and user_name != name and passw != password:
     balance = float(input("enter your balance "))
     print(f"you have been successfuly register\nYour details are, Name: {user_name}, password: {passw}. balance: ${balance:,.2f}.only")
     verified = input("do you want to verified? yes/no ").lower()
     if verified == 'yes' and balance < verification_amount:
        print(f"insufficient fund\nkindly deposite minimum of ${verification_amount:,.2f}.only to verified")
     elif verified == 'yes':
        balance -= verification_amount
        print(f"you have been succesfully verified your balance is ${balance:,.2f}")
        is_verified = True
        print(f"your profile now. Name: {user_name}\npassword: {passw} balance:${balance:,.2f}.only is_verified: {is_verified}")
        New_user = (('Name',user_name),('password',passw),('Balance',balance),('is_verified',is_verified))
        new_user = dict(New_user) #typecasting a tuple to a dictionary
        User_db = dict(user_db)
        print('Old User',User_db)
        print('New User Added',new_user)  
        print('verification successful!!!')
     else: 
        if verified == 'no':
           print(f'Goodbye you have not been verified!!\nkindly pay the sum of ${verification_amount}.only to get verified')
           quit()
  elif command == 'register' and user_name == name:
     print("user already exist")
  else:
    if command == "register" and passw  == password:
       print('password already exit')
    else:
